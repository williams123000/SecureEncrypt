...
export default function FileEncryptionApp() {
    ...
    return (
        <div className={`min-h-screen flex flex-col ${darkMode ? 'bg-zinc-900 text-zinc-200' : 'bg-gray-50'}`}>
            <Toaster position="top-right" reverseOrder={false} />
            <header className={`shadow-sm p-4 border-b ${darkMode ? 'bg-zinc-800 text-zinc-200' : 'bg-white'}`}>
                <div className="max-w-4xl mx-auto flex items-center justify-between">
                    <div className="flex items-center">
                        <Shield className={`mr-2 ${darkMode ? 'text-blue-400' : 'text-blue-600'}`} size={24} />
                        <h1 className={`text-xl font-bold ${darkMode ? 'text-zinc-100' : 'text-gray-900'}`}>SecureEncrypt</h1>
                    </div>
                    <div className="flex items-center space-x-4">
                        <button
                            onClick={() => setDarkMode(!darkMode)}
                            className={`p-2 rounded-full ${darkMode ? 'hover:bg-zinc-700' : 'hover:bg-gray-200'}`}
                        >
                            {darkMode ? '☀️' : '🌙'}
                        </button>
                        <button
                            onClick={handleSignOut}
                            className={`px-4 py-2 bg-red-600 text-white rounded-md hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 ${darkMode ? 'focus:ring-offset-zinc-800' : 'focus:ring-offset-white'} disabled:opacity-50 disabled:cursor-not-allowed`}
                        >
                            Cerrar sesión
                        </button>
                    </div>
            ...
            <main className="flex-1 p-6">
                <div className={`max-w-4xl mx-auto rounded-lg shadow-md overflow-hidden ${darkMode ? 'bg-zinc-800 text-zinc-200' : 'bg-white'}`}>
                    <div className="p-6">
                        <h2 className={`text-2xl font-bold mb-4 flex items-center ${darkMode ? 'text-zinc-100' : 'text-gray-900'}`}>
                            <Lock className="mr-2 text-blue-600 dark:text-blue-400" size={20} />
                            Encriptación Segura de Archivos
                        </h2>

                        <Alert className={`mb-6 ${darkMode ? 'bg-zinc-700 text-zinc-200 border-zinc-600' : ''}`}>
                        ...
                        <div className="space-y-6">
                            <div className={`border-2 border-dashed rounded-lg p-8 text-center hover:border-blue-500 transition-colors duration-200 ${darkMode ? 'border-zinc-700' : 'border-gray-300'}`}>
                                <div className="flex flex-col items-center justify-center">
                                    <FileText className={`h-12 w-12 mb-4 ${file ? 'text-blue-600 dark:text-blue-400' : 'text-gray-400 dark:text-zinc-500'}`} />
                                    {file ? (
                                        <div className="text-sm">
                                            <p className={`font-medium ${darkMode ? 'text-zinc-100' : 'text-gray-900'}`}>{file.name}</p>
                                            <p className={`${darkMode ? 'text-zinc-400' : 'text-gray-500'}`}>{Math.round(file.size / 1024)} KB</p>
                                            {filePreview && (
                                                <div className="mt-2">
                                                    ...
                                                    {filePreview.type === 'pdf' && (
                                                        <p className={`${darkMode ? 'text-zinc-400' : 'text-gray-500'} italic`}>PDF seleccionado (vista previa no disponible en línea)</p>
                                                    )}
                                                    {filePreview.type === 'text' && (
                                                        <p className={`text-xs break-words ${darkMode ? 'text-zinc-300' : 'text-gray-600'}`}>{filePreview.content}...</p>
                                                    )}
                                                </div>
                                            )}
                                        </div>
                                    ) : (
                                        <p className={`${darkMode ? 'text-zinc-400' : 'text-gray-500'}`}>Arrastra y suelta tu archivo aquí o haz clic para seleccionar</p>
                                    )}
                                    <button
                            ...
                            <div className="space-y-2">
                                <label htmlFor="password" className={`block text-sm font-medium flex items-center ${darkMode ? 'text-zinc-200' : 'text-gray-700'}`}>
                                    <Key className={`mr-1 h-4 w-4 ${darkMode ? 'text-zinc-400' : 'text-gray-500'}`} />
                                ...
                                <input
                                    ...
                                    className={`w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 ${darkMode ? 'bg-zinc-700 text-zinc-200 border-zinc-600' : 'border-gray-300'}`}
                                />
                                <label htmlFor="confirm-password" className={`block text-sm font-medium flex items-center ${darkMode ? 'text-zinc-200' : 'text-gray-700'}`}>
                                    <Key className={`mr-1 h-4 w-4 ${darkMode ? 'text-zinc-400' : 'text-gray-500'}`} />
                            ...
                            {progress > 0 && (
                                <div className="w-full mt-4">
                                    <div className={`h-2.5 rounded-full ${darkMode ? 'bg-zinc-700' : 'bg-gray-200'}`}>
                                        <div
                                            className="bg-blue-600 h-2.5 rounded-full transition-all duration-300"
                                            style={{ width: `${progress}%` }}
                                        ></div>
                                    </div>
                                    <p className={`text-sm mt-1 ${darkMode ? 'text-zinc-400' : 'text-gray-600'}`}>{progress}% completado</p>
                                </div>
                            )}

                            <div className="mt-8">
                                <div className="flex items-center justify-between mb-4">
                                    <h3 className={`text-lg font-semibold ${darkMode ? 'text-zinc-100' : 'text-gray-800'}`}>Archivos Almacenados</h3>
                                    <button
                                        onClick={fetchSavedFiles}
                                        className={`flex items-center text-sm ${darkMode ? 'text-zinc-400 hover:text-blue-400' : 'text-blue-600 hover:text-blue-800'}`}
                                    >
                                ...
                                {isLoading ? (
                                    <p className={`text-center py-4 ${darkMode ? 'text-zinc-400' : 'text-gray-500'}`}>Cargando archivos...</p>
                                ) : savedFiles.length > 0 ? (
                                    <div className={`overflow-x-auto border rounded-md ${darkMode ? 'border-zinc-700' : 'border-gray-200'}`}>
                                        <table className={`min-w-full divide-y ${darkMode ? 'divide-zinc-700' : 'divide-gray-200'}`}>
                                            <thead className={darkMode ? 'bg-zinc-700' : 'bg-gray-50'}>
                                                <tr>
                                                    <th className={`px-6 py-3 text-left text-xs font-medium uppercase tracking-wider ${darkMode ? 'text-zinc-300' : 'text-gray-500'}`}>Nombre</th>
                                                    <th className={`px-6 py-3 text-left text-xs font-medium uppercase tracking-wider ${darkMode ? 'text-zinc-300' : 'text-gray-500'}`}>Tamaño</th>
                                                    <th className={`px-6 py-3 text-left text-xs font-medium uppercase tracking-wider ${darkMode ? 'text-zinc-300' : 'text-gray-500'}`}>Fecha de subida</th>
                                                    <th className={`px-6 py-3 text-left text-xs font-medium uppercase tracking-wider ${darkMode ? 'text-zinc-300' : 'text-gray-500'}`}>Acciones</th>
                                                </tr>
                                            </thead>
                                            <tbody className={darkMode ? 'bg-zinc-800 divide-zinc-700' : 'bg-white divide-gray-200'}>
                                                {savedFiles.map((file) => (
                                                    <tr key={file.id}>
                                                        <td className={`px-6 py-4 text-sm font-medium break-words max-w-xs ${darkMode ? 'text-zinc-100' : 'text-gray-900'}`}>
                                                            {file.file_name}
                                                        </td>
                                                        <td className={`px-6 py-4 whitespace-nowrap text-sm ${darkMode ? 'text-zinc-400' : 'text-gray-500'}`}>
                                                            ...
                                                            <div className="flex space-x-2">
                                                                ...
                                                                <TooltipProvider>
                                                                    <Tooltip>
                                                                        ...
                                                                        <TooltipContent>
                                                                            Copiar enlace (válido por 1 hora)
                                                                        </TooltipContent>
                                                                    </Tooltip>
                                                                </TooltipProvider>
                                    ...
                                    <p className={`text-center py-4 border rounded-md ${darkMode ? 'text-zinc-400 border-zinc-700' : 'text-gray-500 border-gray-200'}`}>
            ...
            <footer className={`border-t py-4 ${darkMode ? 'bg-zinc-800 text-zinc-400' : 'bg-white text-gray-500'}`}>
                <div className="max-w-4xl mx-auto px-4 text-center text-sm">
                    SecureEncrypt © {new Date().getFullYear()} - Encriptación segura en el navegador
                </div>
            </footer>
        </div>
    );
